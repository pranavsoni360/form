...
async def send_whatsapp_form(request: Request):
    ...
    if phone_norm and agent_type != "account_opening":
        ...
        if existing_app:
            ...
            # Convert eligibility flag strings ("yes"/"no") to booleans for DB
            def _to_bool(val) -> bool | None:
                if val is None:
                    return None
                return str(val).strip().lower() in ("yes", "true", "1")

            is_salaried_flag = _to_bool(collected.get("is_salaried"))
            individual_purpose_flag = _to_bool(collected.get("individual_purpose"))
            ...
            try:
                ...
                field_map = {
                    ...
                    "is_salaried": str(is_salaried_flag) if is_salaried_flag is not None else None,
                    "individual_purpose": str(individual_purpose_flag) if individual_purpose_flag is not None else None,
                }
