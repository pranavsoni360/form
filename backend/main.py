...
async def aa_statement_initiate(request: Request):
    ...
    # Optional: only the statementupload journey uses it. Blank simply means we
    # go straight to the AA consent flow.
    institution_id = "".join(ch for ch in str(data.get("institution_id") or "") if ch.isdigit())
    ...
    return {"url": result["url"], "request_id": request_id,
            ...
            # Which journey the applicant is actually being sent into, so the
            # form can say what to expect: a direct upload page for their bank,
            # or the AA consent screen where they pick the bank themselves.
            "journey": used_journey}
