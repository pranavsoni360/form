...
-- NOT VALID: existing QA/legacy rows may hold junk codes (test banks created
-- via the create-bank form with garbage input). The rule is enforced on every
-- INSERT/UPDATE from now on; legacy rows are tolerated until cleaned, then this
-- can be VALIDATEd. This is the safe pattern for constraining a table that
-- already contains human-entered data.
ALTER TABLE banks DROP CONSTRAINT IF EXISTS chk_banks_code_format;
ALTER TABLE banks ADD CONSTRAINT chk_banks_code_format
    CHECK (code ~ '^[A-Z0-9_]{2,20}$') NOT VALID;
