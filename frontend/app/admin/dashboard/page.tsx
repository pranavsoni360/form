...
export default function OpsOverviewPage() {
  ...
  const SYSTEM_KPIS = [
    ...
    },
    {
      ...
      hint: errorsLast5m === 0 ? "all clear" : "see /admin/errors",
    },
  ];
        ...
        {loading ? (
          ...
          <div className="grid place-items-center rounded-lg border border-dashed border-border px-6 py-10 text-center">
            ...
            <div className="mt-0.5 text-xs text-muted-foreground">
              Upload a batch from <Link href="/admin/batch" className="text-primary hover:underline">/admin/batch</Link>.
            </div>
          </div>
        ) : (
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <Link href="/admin/live" className="btn-solid">
        ...
        <div className="space-y-1">
          ...
          <CardDescription>
            {activeCount === 0
              ...
              : `${activeCount} call${activeCount === 1 ? "" : "s"} in-flight — hop to /admin/live for the full grid.`}
          </CardDescription>
        </div>
      ...
      <CardContent>
        <div className="grid place-items-center rounded-xl border border-dashed border-border bg-muted/30 px-6 py-10 text-center">
          ...
          <div className="mt-3 text-sm font-semibold">
            {activeCount === 0 ? "Ready for the next batch" : "Cards on /admin/live"}
          </div>
          <div className="mt-1 max-w-sm text-xs text-muted-foreground">
            Upload a CSV from{" "}
            <Link href="/admin/batch" className="font-medium text-primary hover:underline">
              Batch
            </Link>{" "}
            — the dispatcher picks it up within seconds, and live cards appear at{" "}
            <Link href="/admin/live" className="font-medium text-primary hover:underline">
              /admin/live
            </Link>
            .
          </div>
          <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
            <Link href="/admin/batch" className="btn-gradient">
              <PlusCircle className="h-4 w-4" />
              Upload a batch
            </Link>
            <Link href="/admin/live" className="inline-flex items-center gap-2 rounded-xl border border-border bg-card px-4 py-2 text-sm font-medium hover:bg-muted">
              View live grid
