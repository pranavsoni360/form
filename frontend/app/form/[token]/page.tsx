...
useEffect(() => {
  ...
  if (phone && phone.replace(/\D/g, '').length >= 10) {
    ...
    setTimeout(() => autoSendOTP(digits), 800);
  }
}, []);

const autoSendOTP = async (phone: string) => {
  setOtpSending(true);
  setOtpError('');
  try {
    ...
    if (data.status === 'otp_sent') {
      ...
      setOtpError(data.detail || 'Failed to send OTP. Enter your number manually.');
    }
  } catch {
    setOtpError('Could not auto-send OTP. Please enter your number.');
  } finally {
    setOtpSending(false);
  }
};
